...
- **WSL2 idle shutdown**: WSL2 terminates the distro a few seconds after its last session closes (systemd does not prevent this), and Docker plus postgres die with it. `docker compose up -d` from a Windows terminal is such a session, so on its own postgres would vanish ~10 s after it returns and the api would log `ECONNREFUSED` on every query. `pnpm run dev` / `dev:api` therefore hold an idle `wsl.exe -- sleep infinity` session open for as long as the api runs. If you start the api some other way (`pnpm run start:dev`, or `db:migrate` against WSL2's postgres), keep a WSL terminal window open while you do.
