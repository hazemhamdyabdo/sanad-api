...
import {
  EMAIL_PROVIDER,
  type EmailProvider,
} from '../../integrations/email/email.interface.js';
...
import {
  summarize,
  toApplicationDto,
  type ApplicationBatchDto,
  type ApplicationDto,
  type ApplicationsListDto,
...
function bulletsOf(entry: unknown): string[] {
  ...
  return Array.isArray(value)
    ? value.filter((bullet): bullet is string => typeof bullet === 'string')
    : [];
}
...
function applyTailoring(
  ...
  const withBullets = (entries: unknown[], bullets: string[][]) =>
    entries.map((entry, index) =>
      ...
      Array.isArray((entry as Record<string, unknown>).bullets)
        ? { ...entry, bullets: bullets[index] }
        : entry,
    );
  const skillsByName = new Map(
    cv.skills.map((skill) => [str(skill, 'name'), skill]),
  );
  const orderedSkills: unknown[] = tailored.skillOrder
  ...
  return {
    ...
    skills: [
      ...orderedSkills,
      ...cv.skills.filter((skill) => !orderedSkills.includes(skill)),
    ],
  };
}
...
export class ApplicationsService implements OnModuleInit, OnModuleDestroy {
  ...
  onModuleInit(): void {
    this.timer = setInterval(() => {
      this.resumeStale().catch((error) =>
        this.logger.error(
          'Application resume sweep failed',
  ...
  async create(
    deviceId: string,
    jobIds: string[],
  ): Promise<ApplicationBatchDto> {
    if (!(await this.cvService.existsForDevice(deviceId))) {
      throw new AppError(
        'NOT_FOUND',
        'لسه معندكش سيرة ذاتية — اعملها الأول وبعدين قدّم',
        { retryable: false },
      );
    }
    ...
    if (jobs.length !== jobIds.length) {
      throw new AppError(
        'INVALID_REQUEST',
        'في وظيفة من اللي اخترتها مبقتش موجودة، حدّث القايمة وجرب تاني',
        { retryable: false },
      );
    }
    ...
    const processing = await this.repository.inDeviceLock(
      deviceId,
      async (manager) => {
        const existing = new Map(
          (await this.repository.findForJobs(deviceId, jobIds, manager)).map(
            (application) => [application.jobId, application],
          ),
        );
          ...
          toProcess.push(
            Object.assign(
                ...
                }),
              {
                ...
                listingUrl: job.link,
                recipientEmail:
                  job.applyMethod === 'email' ? job.applyEmail : null,
                tailoredCv: null,
                cvTailored: false,
                errorCode: null,
                errorDetail: null,
        ...
        await this.repository.createBatch(
          {
            ...
            alreadyAppliedIds: alreadyApplied.map(
    ...
    if (processing) {
      ...
      void this.processBatch(batchId).catch((error) =>
        this.logger.error(
          `Batch ${batchId} failed unexpectedly`,
          error instanceof Error ? error.stack : error,
        ),
      );
    }
  ...
  async getBatch(
    ...
    if (!batch) {
      throw new AppError('NOT_FOUND', 'مش لاقيين التقديم ده', {
        retryable: false,
      });
    }
    const order = (ids: string[]) => (applications: Application[]) =>
      ids
        .map((id) => applications.find((application) => application.id === id))
        .filter((application) => application !== undefined);
    const applications = order(batch.applicationIds)(
      await this.repository.findByIds(batch.applicationIds),
    ).map(toApplicationDto);
    if (
      batch.status !== 'done' &&
      applications.every((application) => application.status !== 'processing')
    ) {
    ...
    const alreadyApplied = order(batch.alreadyAppliedIds)(
      await this.repository.findByIds(batch.alreadyAppliedIds),
    ).map(toApplicationDto);
  ...
  async list(deviceId: string): Promise<ApplicationsListDto> {
    const applications = (await this.repository.findAllForDevice(deviceId)).map(
      toApplicationDto,
    );
  ...
  async markOpened(deviceId: string, id: string): Promise<ApplicationDto> {
    ...
    if (application.method !== 'external') {
      throw new AppError(
        'INVALID_REQUEST',
        'التقديم ده اتبعت بالإيميل، مفيش إعلان تفتحه',
        { retryable: false },
      );
    }
    if (application.status !== 'prepared' && application.status !== 'opened') {
      throw new AppError('INVALID_REQUEST', 'الوظيفة دي لسه بتتجهز', {
        retryable: true,
      });
    }
  ...
  async markSubmitted(deviceId: string, id: string): Promise<ApplicationDto> {
    ...
    if (application.method !== 'external') {
      throw new AppError(
        'INVALID_REQUEST',
        'التقديم ده اتبعت بالإيميل، مفيش حاجة تكمّلها',
        { retryable: false },
      );
    }
    if (
      ...
      throw new AppError(
        'INVALID_REQUEST',
        'الوظيفة دي لسه متجهزتش، استنى الـ CV يخلص الأول',
        { retryable: application.status === 'processing' },
      );
    }
  ...
  async getCvPdf(
    ...
    if (!application.tailoredCv) {
      throw new AppError('NOT_FOUND', 'الـ CV بتاع الوظيفة دي لسه بيتجهز', {
        retryable: true,
      });
    }
    return {
      file: this.cvService.renderPdf(application.tailoredCv).file,
      filename: applicationCvFilename(application),
    };
  }
  ...
  private async processBatch(batchId: string): Promise<void> {
    ...
    const pending = (
      ...
      (application) =>
        application.status === 'processing' && !this.active.has(application.id),
    );

    if (pending.length) {
      // One CV snapshot for the whole batch — every job in one request gets tailored from the same CV.
      const cv = await this.cvService
        .getForDevice(batch.deviceId)
        .catch(() => null);
      let next = 0;
      const worker = async () => {
    ...
    if (
      (await this.repository.countProcessingInBatch(batchId)) === 0 &&
      batch.status !== 'done'
    ) {
      batch.status = 'done';
      batch.completedAt = new Date();
    ...
    try {
      if (!cv) {
        return await this.fail(
          application,
          'internal',
          'The device has no CV any more.',
        );
      }
      const [job] = application.jobId
        ? await this.jobsService.findJobsByIds([application.jobId])
        : [];
      if (!job) {
        return await this.fail(
          application,
      ...
      if (!application.tailoredCv) {
        ...
        await this.repository.save(application);
        const tailored = await this.cvTailorService.tailor(toTailorInput(cv), {
          title: job.title,
          company: job.company,
          description: toPlainText(job.snippet),
        });
        ...
        application.tailoredCv = contactForJobCountry(
          applyTailoring(cv, tailored),
          job.country,
        );
  ...
  private async send(application: Application): Promise<void> {
    ...
    if (!replyTo || !EMAIL_PATTERN.test(replyTo)) {
      return this.fail(
        application,
        'no_candidate_email',
        'The CV has no valid email to use as Reply-To.',
      );
    }
    if (!application.recipientEmail) {
      return this.fail(
        application,
        'job_unavailable',
        'An email-method application with no recipient address.',
      );
    }
    ...
    const content = buildApplicationEmail(cv, {
      title: application.jobTitle,
      company: application.company,
    });
    const fromName = headerSafe(
      `${cv.name?.trim() || 'Candidate'} via ${this.configService.get<string>('email.fromName', 'Sanad')}`,
    );
    ...
    for (let attempt = 0; attempt < SEND_ATTEMPTS; attempt++) {
      try {
        const sent = await this.email.send({
          ...
          html: content.html,
          attachments: [
            { filename, content: file, contentType: 'application/pdf' },
          ],
        ...
        this.logger.warn(
          `Sending application ${application.id} failed on attempt ${attempt + 1}: ${lastError}`,
        );
  ...
  private async resumeStale(): Promise<void> {
    const stale = (
      await this.repository.findStaleProcessing(
        new Date(Date.now() - STALE_AFTER_MS),
      )
    ).filter((application) => !this.active.has(application.id));
    for (const batchId of new Set(
      stale.map((application) => application.batchId),
    )) {
