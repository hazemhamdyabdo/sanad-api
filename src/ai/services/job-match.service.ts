...
import { EXTRACTION_LLM_PROVIDER, type LlmMessage, type LlmProvider } from '../../integrations/llm/llm.interface.js';
import { buildJobMatchPrompt } from '../prompts/job-match.prompt.js';
import { isArabicLine, jobMatchEnvelopeSchema, jobMatchItemSchema, type JobMatchExplanation, type MatchCandidate, type MatchJob } from '../schemas/job-match.schema.js';
...
const KNOWN_REJECTIONS: string[] = [REJECTION_SHAPE, REJECTION_LANGUAGE, REJECTION_JOB_ID];
...
function buildCorrection(rejections: Set<string>, missing: MatchJob[]): string {
  ...
  if (rejections.has(REJECTION_SHAPE)) {
    points.push('الرد لازم يكون كائن JSON واحد فيه "matches" بالشكل المطلوب، من غير أي نص قبله أو بعده.');
  }
  ...
  if (rejections.has(REJECTION_JOB_ID)) {
    points.push('كل "jobId" من القايمة لازم يظهر مرة واحدة بالظبط، بنفس الـ id المكتوب، ومن غير وظايف مش في القايمة.');
  }
  if ([...rejections].some((rejection) => !KNOWN_REJECTIONS.includes(rejection))) {
    points.push('في عناصر مش مطابقة للشكل المطلوب: "match" رقم من 0 لـ 100، و"whyMatch" و"gaps" قوايم سطور قصيرة (من 2 لـ 120 حرف).');
  }
  if (!points.length) {
...
export class JobMatchService {
  ...
  private async explainBatch(candidate: MatchCandidate, batch: MatchJob[]): Promise<JobMatchExplanation[]> {
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      if (previous) {
        messages.push({ role: 'assistant', content: previous.raw }, { role: 'user', content: buildCorrection(previous.rejections, missing) });
      }
      ...
      for (const item of envelope.data.matches) {
        ...
        if (!result.success) {
          ...
          rejections.push(result.error.issues.map((issue) => `${issue.path.join('.')}: ${issue.message}`).join(', '));
          continue;
        }
        if (!wanted.has(result.data.jobId) || explained.has(result.data.jobId)) {
        ...
        englishLines += result.data.whyMatch.length - arabicWhyMatch.length + (result.data.gaps.length - arabicGaps.length);
        ...
        const whyMatch = arabicWhyMatch.filter((line) => isTraceableReason(line, facts, candidate.yearsOfExperience));
        untracedReasons += arabicWhyMatch.length - whyMatch.length;
        explained.set(result.data.jobId, {
          ...result.data,
          whyMatch,
          gaps: dropJuniorGaps(dropMetYearsGaps(arabicGaps, candidate.yearsOfExperience), candidate.seniority),
        });
      }

      const unanswered = missing.filter((job) => !explained.has(job.id)).length;
      if (rejections.length || unanswered) {
        const reasons = [...new Set(rejections)].join(' | ') || 'none (jobs missing from the reply)';
        this.logger.warn(`Job match attempt ${attempt + 1}: ${unanswered} of ${missing.length} job(s) still unexplained; rejected ${rejections.length} item(s) — ${reasons}`);
      }
      previous = { raw, rejections: new Set(rejections) };
    }

    if (englishLines) {
      this.logger.warn(`Job match: dropped ${englishLines} all-English line(s) from otherwise valid explanations.`);
    }
    if (untracedReasons) {
