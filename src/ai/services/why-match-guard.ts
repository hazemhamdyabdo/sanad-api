...
export function candidateFactsText(candidate: MatchCandidate): string {
  ...
  const extra = ABBREVIATIONS.flatMap(([short, long]) => {
    if (new RegExp(`(^|[^a-z0-9])${short}($|[^a-z0-9])`).test(text))
      return [long];
...
function isTraced(
  ...
  if (/^\d+(\.\d+)?$/.test(term)) {
    return (
      Number(term) === yearsOfExperience ||
      new RegExp(`(^|[^0-9])${escapeRegExp(term)}($|[^0-9])`).test(facts)
    );
  }
...
function isInFacts(form: string, facts: string): boolean {
  ...
  if (form.length <= 3) {
    return new RegExp(`(^|[^a-z0-9])${escapeRegExp(form)}($|[^a-z0-9])`).test(
      facts,
    );
  }
  // "fine-tuning" is the CV's "fine tuned" or "finetuned" too.
  const spellings = form.includes('-')
    ? [form, form.replace(/-/g, ' '), form.replace(/-/g, '')]
    : [form];
...
export function isTraceableReason(
  line: string,
  facts: string,
  yearsOfExperience: number | null,
): boolean {
  ...
  return (
    terms.length > 0 &&
    terms.every((term) => isTraced(term, facts, yearsOfExperience))
  );
}
