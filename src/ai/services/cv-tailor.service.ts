...
} from '../schemas/cv-tailor.schema.js';
...
function acceptBullets(
  ...
  return proposed.some((bullet) => introducesNewFacts(bullet, entrySource))
    ? null
    : proposed;
}
...
function acceptSkillOrder(skills: string[], proposed: string[]): string[] {
  const byKey = new Map(
    skills.map((skill) => [skill.trim().toLowerCase(), skill]),
  );
...
export class CvTailorService {
  ...
  async tailor(
    ...
    const hasBullets = [...cv.experience, ...cv.projects].some(
      (entry) => entry.bullets.length > 1,
    );
    ...
    for (let attempt = 0; attempt < MAX_ATTEMPTS; attempt++) {
      ...
      try {
        raw = await this.llm.complete({
          messages: buildCvTailorPrompt(cv, job),
          temperature: 0,
          jsonMode: true,
          maxTokens: TAILOR_MAX_TOKENS,
          timeoutMs: TAILOR_CALL_TIMEOUT_MS,
        });
      } catch (error) {
        this.logger.warn(
          `CV tailoring call failed on attempt ${attempt + 1}: ${error instanceof Error ? error.message : String(error)}`,
        );
        continue;
      }
      ...
      try {
        ...
        this.logger.warn(
          `CV tailoring output was not valid JSON on attempt ${attempt + 1}.`,
        );
        continue;
      }
      ...
      if (!result.success) {
        this.logger.warn(
          `CV tailoring output failed validation on attempt ${attempt + 1}: ${result.error.issues.map((issue) => `${issue.path.join('.')}: ${issue.message}`).join(', ')}`,
        );
        continue;
      }

      const proposedExperience = new Map(
        result.data.experience.map((entry) => [entry.index, entry.bullets]),
      );
      const proposedProjects = new Map(
        result.data.projects.map((entry) => [entry.index, entry.bullets]),
      );
      ...
      const experienceBullets = cv.experience.map((entry, index) => {
        const accepted = acceptBullets(
          entry.bullets,
          proposedExperience.get(index),
          [entry.title, entry.company, ...entry.bullets].join('\n'),
        );
        if (
          accepted?.some(
            (bullet, position) => bullet !== entry.bullets[position],
          )
        )
          acceptedEntries++;
      ...
      const projectBullets = cv.projects.map((entry, index) => {
        const accepted = acceptBullets(
          entry.bullets,
          proposedProjects.get(index),
          [entry.title, entry.description, ...entry.bullets].join('\n'),
        );
        if (
          accepted?.some(
            (bullet, position) => bullet !== entry.bullets[position],
      ...
      if (rejectedEntries) {
        this.logger.warn(
          `CV tailoring: kept the original bullets for ${rejectedEntries} entr${rejectedEntries === 1 ? 'y' : 'ies'} (wrong bullet count or a fact not in the CV).`,
        );
      }
      const skillsChanged = skillOrder.some(
        (skill, index) => skill !== cv.skills[index],
      );
      return {
        experienceBullets,
        projectBullets,
        skillOrder,
        tailored: acceptedEntries > 0 || skillsChanged,
      };
    }

    this.logger.warn(
      'CV tailoring failed after all attempts — using the CV as written.',
    );
    return untouched;
  }
}
