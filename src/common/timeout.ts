...
export function isTimeoutError(error: unknown): boolean {
  return error instanceof TimeoutError || (error instanceof Error && (error.name === 'TimeoutError' || error.name === 'AbortError'));
}
 ...
 */
export async function fetchWithTimeout(url: string, init: RequestInit, ms: number, what: string): Promise<Response> {
  try {
