...
pgvector: the `AddJobMatching` migration runs `CREATE EXTENSION IF NOT EXISTS vector`, so the Postgres image must ship the extension's binaries. Railway's own `postgres-ssl` template image now installs `postgresql-17-pgvector` (see `Dockerfile.17` in [railwayapp-templates/postgres-ssl](https://github.com/railwayapp-templates/postgres-ssl)), even though the [Railway Postgres docs](https://docs.railway.com/databases/postgresql) still say the default templates carry no extensions. Either the stock Postgres template on a current image or the upstream `pgvector/pgvector:0.8.6-pg17-bookworm` image below works; there is no need to build our local Postgres Dockerfile. If the extension is missing, the migration fails with `extension "vector" is not available` and nothing is committed (see the troubleshooting section). Railway describes these database templates as unmanaged: it provides the hosting, but you own configuration and maintenance. [Upstream image tags](https://github.com/pgvector/pgvector#docker).
...
7. **Review Settings, then deploy.** `railway.json` selects the Dockerfile, the start command (migrations, then `node dist/main.js`), `/api/v1/health`, and one replica. The Dockerfile `CMD` is the same command, so migrations run even if Railway ignores the config file. In **sanad-api → Settings → Deploy**, leave **Custom Start Command** and **Pre-deploy Command** empty (a dashboard start command would replace the migrate-then-start line) and confirm the **Config-as-code** panel shows `railway.json` was read. Do not replace these with `pnpm dev`, Compose, or `nest deploy`. Leave `PORT` unset so Railway supplies it. Apply all pending changes. In deployment logs, the first lines are TypeORM's migration output (`Migration ... has been executed successfully` or `No migrations are pending`), followed by Nest startup and the email provider reporting **REDIRECTED**. A migration failure exits the container before the API starts; Railway retries up to five times, then marks the deployment crashed. Correct the cause and redeploy. Migrations use the private network and require a ready database.
...
The container start command is `node node_modules/typeorm/cli.js migration:run -d dist/database/data-source.js && exec node dist/main.js`, defined once in the Dockerfile `CMD` and repeated as `startCommand` in `railway.json` (Railway runs a Dockerfile start command in exec form, hence the `/bin/sh -c` wrapper there). The data source discovers compiled JS migrations/entities in `dist`; local TS migration commands continue to work. TypeORM tracks applied migrations in the `migrations` table; repeat deploys apply only new ones, and a restart with nothing pending costs about a second. `synchronize` remains false. An earlier revision used Railway's pre-deploy command instead; it was dropped because a deployment shipped where the step never ran and the API booted against an empty schema (`relation "role_ingestion_cache" does not exist` every 60 seconds from the job ingestion sweep). Running migrations in the start command cannot be skipped by a dashboard setting or an unread config file.

### Troubleshooting migrations

- **`relation "…" does not exist` in API logs**: the schema is behind. With the current start command this cannot happen unless a dashboard **Custom Start Command** overrides it; clear it in Settings → Deploy and redeploy.
- **`extension "vector" is not available` / `could not open extension control file`** from `AddJobMatching`: the Postgres image lacks pgvector. TypeORM runs all pending migrations in one transaction, so on a fresh database every table is rolled back, not just the matching ones; the API exits and Railway retries. Fix: give the Postgres service an image that ships pgvector (upstream `pgvector/pgvector:0.8.6-pg17-bookworm`, or Railway's current `postgres-ssl` image). Since no migration committed, the database is still empty and can simply be replaced: create the new Postgres service, point the `${{Postgres.*}}` references at it, and redeploy the API. If the same volume is reused with a pgvector-capable image of the **same major version**, existing data survives and the next start applies the migrations.
- **`ECONNREFUSED` / `ENOTFOUND`** at start: Postgres is not ready or the private host reference is wrong; Railway retries the container up to five times, which usually covers a slow Postgres boot.
